...
classDef entry fill:#eff6ff,stroke:#3b82f6,stroke-width:1px,color:#1e3a8a
classDef platform fill:#f0fdf4,stroke:#22c55e,stroke-width:1px,color:#14532d
classDef runtime fill:#faf5ff,stroke:#a855f7,stroke-width:1px,color:#581c87
classDef model fill:#fff1f2,stroke:#e11d48,stroke-width:1px,color:#881337
classDef store fill:#f8fafc,stroke:#64748b,stroke-width:1px,color:#334155
classDef orchestr fill:#f5f3ff,stroke:#7c3aed,stroke-width:1px,color:#4c1d95
...
subgraph L2["平台层"]
direction TB
D["ClawLoops 控制平面(Web UI + API)"]:::platform
end
...
direction TB
I["LiteLLM"]:::model
J["vLLM / Ollama / 上游 API"]:::model
I --> J
end

subgraph L6["资料与存储层"]
direction LR
N["资料与 Skill 管理服务(上传/权限/版本/加载)"]:::store
K[(PostgreSQL元数据)]:::store
L[(宿主机磁盘持久化资料目录)]:::store
D -->|元数据| K
D -->|文件落盘| L
D -->|上传/权限控制| N
N -->|写入/读取| L
...
subgraph L4["运行时层"]
direction TB
G["每用户 OpenClaw Runtime(独立工作区)"]:::runtime
end

subgraph L3["编排层"]
direction TB
E["运行时编排器"]:::orchestr
F["Runtime Manager"]:::orchestr
E --> F
end
...
style L5 fill:#ffffff,stroke:#cbd5e1,stroke-width:1px
style L6 fill:#ffffff,stroke:#cbd5e1,stroke-width:1px
...
| 平台层  | ClawLoops 控制平面 + Web UI + 文件管理服务 + 文件上传服务 | 用户管理、工作区入口、管理治理、运行时业务状态管理、文件操作API、文件上传处理 |
| 编排层  | Runtime Manager                           | 容器生命周期管理、配置渲染、状态监控、容器文件操作、文件上传处理         |
| 运行时层 | 每用户 OpenClaw Runtime + 容器文件系统             | 用户隔离工作区、Runtime 配置、交互式 AI 环境、文件存储与管理     |
| 模型层  | LiteLLM + 上游模型提供商                         | 统一模型访问、凭据代理、模型路由与聚合                      |
| 数据层  | 数据存储                                      | 用户、工作区、邀请、运行时元数据持久化                      |
